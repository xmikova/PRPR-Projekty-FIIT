...
int o(FILE **fr, unsigned long long int *p1, char ***p2, char ***p3, double *p4, char ***p5, unsigned long long int *p6, unsigned int *zaznamy) {
  ...
  for (i = 0; i < cntr - 1; i++) {
      for (j = 0; j < cntr - i - 1; j++) {
        if (((p6[pole_poz[j]] > p6[pole_poz[j + 1]]) && (atoi((*p5)[pole_poz[j]]) <= atoi((*p5)[pole_poz[j + 1]]))) || ((p6[pole_poz[j]] == p6[pole_poz[j + 1]]) && (atoi((*p5)[pole_poz[j]]) > atoi((*p5)[pole_poz[j + 1]])))) {
            pom = pole_poz[j + 1];
...
int h(FILE **fr, unsigned long long int *p1, char ***p2, char ***p3, double *p4, char ***p5, unsigned long long int *p6, unsigned int *zaznamy) {
  char tmv[10];
  int i = 0, j = 0, cntr = 0, pom = 0, cntr_hist = 0, min = 0,  max = 5, vel_pol = 0, pos = 0;
...
int c(FILE **fr){
  ...
  do {
        fscanf(*fr, "%llu \n %s \n %s \n %lf \n %s \n %llu \n", &id, modul, typ, &hodnota, cas, &datum);
        if ((id % 11 != 0) && (floor(log10(id) + 1) != 11)){
  ...
  int main() {
    ...
    do {
      scanf("%c", &pismeno);
      switch (pismeno) {
        case 'v': v(&fr, p1, &p2, &p3, p4, &p5, p6, &zaznamy); break; //funkcia na vypis zaznamov
        case 'o': o(&fr, p1, &p2, &p3, p4, &p5, p6, &zaznamy); break;//funkcia na vypis odmeny
        case 'n': n(&fr, &p1, &p2, &p3, &p4, &p5, &p6, &zaznamy); break;//funkcia na vytvorenie dinamickeho pola
        case 'h': h(&fr, p1, &p2, &p3, p4, &p5, p6, &zaznamy);; break;//vypise spz specialnym sposobom
        case 's': s(&fr, p1, &p2, &p3, p4, &p5, p6, &zaznamy); break;//vypise najcastejsie sa vyskitujuci sa znak a jeho pocetnost
        case 'r': r(&fr, p1, &p2, &p3, p4, &p5, p6, &zaznamy); break;
